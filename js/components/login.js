...
function initLogin() {
  ...
  on("#login-btn", "click", async () => {
    ...
    const res = await fetch(
      "https://ancient-term-4335.danporter-36a.workers.dev/api/login",
      {
        method: "POST",
...
function initRegister() {
  ...
  on("#reg-btn", "click", async () => {
    ...
    const res = await fetch(
      "https://ancient-term-4335.danporter-36a.workers.dev/api/register",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    if (!res.ok) {
      emailErr.textContent =
        "Email already registered - Please try a different E-mail";
      return;
    }
    setContent(`
      <section class="panel login"> 
